...
set K 65
set RTT 0.0001
set simulationTime 1.8

set startMeasurementTime 0.0001
set stopMeasurementTime 0.999
...
for {set i 0} {$i < $N} {incr i} {
    ...
    if {$i <= 3} {
        $tcp($i) set dctcp_ true
        $tcp($i) set dctcp_g_ $DCTCP_g_
    }
    if {$i >= 4} {
        $tcp($i) set d2tcp_ true
        $tcp($i) set d2tcp_gamma_ $D2TCP_gamma_
        $tcp($i) set d2tcp_deadline_ [expr ($i + 1) * 0.2]
        $tcp($i) set d2tcp_total_bytes_ 40000000.0
    }
